...
# 获取数据库文件的绝对路径
db_file_path = 'C:/GitTest/yolov8-flask-vue-deploy/detection-backend/instance/detection.sqlite'
...
#特定模型（暂时）
specific = ['test','test1','test2','test3','test4','test5','test6']
"""
...
def switch_model():
    ...
    try:
        ...
        if name in specific:
            try:
             app.config["model"]: YOLO = torch.hub.load('C:/Users/CecilRiver/.cache/torch/hub/ultralytics_yolov5_master',
                                                     'custom',
                                                     path,
                                                     source='local')
            except Exception as e:
...
def detection():
    ...
    try:
        ...
        length = 0
        info = []
        if (app.config["model_json"]["model_name"] in specific):
            ...
            for result in results.pandas().xyxy[0].values:
                pic_data = {
                    ...
                    'y2': result[3],
                    'conf': result[4],
                    'cls': cls[int(result[5])]
                }
                length += 1
                info.append(pic_data)
                results.render()
            res_plotted = results.ims[0]
